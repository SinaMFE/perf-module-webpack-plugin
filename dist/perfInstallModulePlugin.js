...
module.exports = function () {
    function perfInstallModulePlugin(options) {
        _classCallCheck(this, perfInstallModulePlugin);

        this.options = options || {};
        this.PackageJSON = require(path.join(process.cwd(), "./package.json"));
    }

    _createClass(perfInstallModulePlugin, [{
        key: "apply",
        value: function apply(compiler) {
            var me = this;
            compiler.plugin("compilation", function (compilation, params) {
                compilation.mainTemplate.plugin("local-vars", function (source, chunk, hash) {
                    return this.asString([source, "// The module cache", `let countTimeMap = {};`, `let RecordTime=[];
                    ...
                    `, `setTimeout(function(){
                        ...
                        //上报
                    },5000)`, "var installedModules = {};"]);
                });
                compilation.mainTemplate.plugin("require", function (source, chunk, hash) {
                    return this.asString(["// Check if module is in cache", "if(installedModules[moduleId]) {", this.indent("return installedModules[moduleId].exports;"), "}", "// Create a new module (and put it into the cache)", "var module = installedModules[moduleId] = {", this.indent(this.applyPluginsWaterfall("module-obj", "", chunk, hash, "moduleId")), "};", "", this.asString(["// Execute the module function", `
                            let start = new Date();
                            RecordTime.push({
                                moduleId:moduleId,
                                value:start,
    ...
    return perfInstallModulePlugin;
}();